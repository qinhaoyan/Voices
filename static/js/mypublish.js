...
$(function(){
	var url = /#(\d)$/.exec(window.location.href);
	getData(url[1]);
})

weimg = ['/static/img/1.JPG','/static/img/2.JPG','/static/img/3.JPG']
//加载数据
function getData(type){
	var addpage;
	//console.log($("#page"+type+"").html()!='')
	if($("#page"+type+"").html()!=''){
		return;
	}
	else {
		addpage = $("#page"+type+"");
		$('#page').css('transform',"translate("+parseInt(type-1)*(-10)+"rem,0)")
		$('#nav').addClass("nav"+type);
		$('#nav div:eq('+parseInt(type-1)+')').addClass('active');
		$('#page'+type+'').css('height','auto');
		//createPage(type,data,addpage);
		$.ajax({
			url: '/getmypublish',
			type: 'get',
			dataType: 'json',
...
function createPage(type,data,addpage){
	...
	$.each(data,function(i,cont){
		...
		var article = $("<div class='article'><div class='article_user'><div class='article_user_head'><img src='/media/"+cont.userhead+"'></div><div class='article_user_word'><div class='article_user_word_name'>"+cont.username+"</div><div class='article_user_word_time'>"+cont.time+"</div></div><div class='article_user_fun' data-type = "+type+"><div class='article_user_fun_icon"+type+"'></div><div class='article_user_fun_word"+type+"'>"+cont.type+"</div></div></div><a href='/article/"+cont.url+"'><p class='article_p'>"+cont.article+"</p></a><div class='article_fun'><div class='article_fun_comment'>评论</div><div class='article_fun_like' data-articleid = "+cont.url+" data-like="+like_d+" data-liken="+cont.liken+"><span class='article_fun_like_span'>"+likeo+"</span><span class='article_fun_like_word'>赞</span>（<span class='article_fun_like_wordn'>"+cont.liken+"</span>）</div></div></div>");
